...
# Usage: generate_password [prefix] [length]
# Returns: prefix + random characters (uppercase, lowercase, digits, symbols)
# -----------------------------------------------------------------------------
generate_password() {
    local prefix="${1:-Xshop}"
    local length="${2:-16}"
    local random_part=$(openssl rand -base64 48 | tr -dc 'A-Za-z0-9!@#$%' | head -c "$length")
...
create_role_assignment() {
    ...
    case "$ROLE_NAME" in
        "AcrPull")
            print_error "AcrPull role is not applicable for App Service native deployment"
            return 1
            ;;
...
resource_exists() {
    ...
    case "$resource_type" in
        ...
        "identity")
            ...
            ;;
        "acr")
            print_error "ACR is not used in App Service native deployment"
            return 1
            ;;
        "keyvault")
        ...
        "communication")
            ...
            ;;
        "appservice-plan")
            az appservice plan show --name "$name" --resource-group "$resource_group" &>/dev/null
            ;;
        *)
