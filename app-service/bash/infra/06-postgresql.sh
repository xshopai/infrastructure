...
# Sourced by deploy.sh: source "$SCRIPT_DIR/common.sh"
...
deploy_postgresql() {
    ...
    if az postgres flexible-server show --name "$POSTGRES_SERVER" --resource-group "$RESOURCE_GROUP" &>/dev/null; then
        print_success "PostgreSQL Server exists: $POSTGRES_SERVER (skipping creation)"
    else
...
# Create service databases (required before migrations run at service startup)
    local pg_databases=("audit_service_db" "order_processor_db")
    for db_name in "${pg_databases[@]}"; do
        if az postgres flexible-server db show \
            --database-name "$db_name" \
            --server-name "$POSTGRES_SERVER" \
            --resource-group "$RESOURCE_GROUP" &>/dev/null; then
            print_info "PostgreSQL database already exists: $db_name"
        else
