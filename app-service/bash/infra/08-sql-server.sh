...
EOF
    ...
    # Create service databases (required before EF Core migrations can run)
    local sql_databases=("order_service_db" "payment_service_db")
    for db_name in "${sql_databases[@]}"; do
        if az sql db show --name "$db_name" --server "$SQL_SERVER" --resource-group "$RESOURCE_GROUP" &>/dev/null; then
            print_info "SQL database already exists: $db_name"
        else
            print_info "Creating SQL database: $db_name"
            if az sql db create \
                --name "$db_name" \
                --server "$SQL_SERVER" \
                --resource-group "$RESOURCE_GROUP" \
