  ...
  properties: {
    ...
    siteConfig: {
      ...
      appSettings: [
        ...
        { name: 'Order__DefaultStatus', value: 'pending' }
        { name: 'Order__CancellationWindowHours', value: '24' }
        { name: 'Order__StatusTransitions', value: 'pending,processing,shipped,delivered,cancelled' }
      ]
    }
  }
}
    ...
    accessPolicies: [
      {
        ...
        objectId: orderService.identity.principalId
        permissions: {
          secrets: ['get', 'list']
        }
      }
...
output appServiceName string = orderService.name
output appServiceUrl string = 'https://${orderService.properties.defaultHostName}'
output principalId string = orderService.identity.principalId
